...
export default function ArtClassesPage() {
  const service = SERVICES.find(s => s.slug === 'art-classes');
  ...
  const trailImages = useMemo(() => {
    return GALLERY_ITEMS.filter(item => item.category?.includes('art-classes')).map(item => item.src);
  }, []);
  ...
  return (
    <>
              ...
              <div>
                ...
                <Button asChild size="lg" className="mt-6 bg-accent hover:bg-accent/90 text-accent-foreground shadow-md">
                  <a href={`https://wa.me/919403420909?text=${service.whatsappEnquiryText}`} target="_blank" rel="noopener noreferrer">
                    <MessageSquare className="mr-2 h-5 w-5" /> Enrol in Art Classes
                  </a>
                </Button>
              </div>
              <div
      ...
      {circularGalleryMedia.length > 0 && (
        <SectionWrapper title="From Our Classroom" subtitle="Student Creations" className="bg-muted/40">
          <div className="relative h-[60vh]">
            <CircularGallery items={circularGalleryMedia} />
          </div>
          <div className="mt-12 text-center">
            <Button asChild variant="outline" size="lg" className="border-primary text-primary hover:bg-primary/10">
              <Link href="/gallery?category=art-classes">Discover More Student Art</Link>
            </Button>
          </div>
        </SectionWrapper>
      )}
