...
export default function DiyStationsPage() {
  const service = SERVICES.find(s => s.slug === 'diy-stations');
  // For the gallery, we want to show DIY Station images
  const galleryServiceContent = SERVICES.find(s => s.slug === 'diy-stations');
  ...
  const trailImages = useMemo(() => {
    return GALLERY_ITEMS.filter(item => item.category?.includes('diy-stations')).map(item => item.src);
  }, []);
  ...
  return (
    <>
              ...
              <div>
                ...
                <Button asChild size="lg" className="mt-6 bg-accent hover:bg-accent/90 text-accent-foreground shadow-md">
                  <a href={`https://wa.me/919403420909?text=${service.whatsappEnquiryText}`} target="_blank" rel="noopener noreferrer">
                    <MessageSquare className="mr-2 h-5 w-5" /> Book a DIY Station
                  </a>
                </Button>
              </div>
              <div
      ...
      {circularGalleryMedia.length > 0 && (
        <SectionWrapper title="DIY Stations in Action" subtitle="Creativity in Motion" className="bg-muted/40">
          <div className="relative h-[60vh]">
            <CircularGallery items={circularGalleryMedia} />
          </div>
           <div className="mt-12 text-center">
            <Button asChild variant="outline" size="lg" className="border-primary text-primary hover:bg-primary/10">
              <Link href="/gallery?category=diy-stations">More DIY Station Photos</Link>
            </Button>
          </div>
        </SectionWrapper>
      )}
