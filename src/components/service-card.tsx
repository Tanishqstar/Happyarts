...
export function ServiceCard({ icon: imageUrl, title, description, link, onImageClick }: ServiceCardProps) {
  return (
    <div className="glow-wrapper h-full">
      <Card className="flex flex-col h-full shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out overflow-hidden rounded-xl border-border/60">
        <CardHeader className="items-center text-center p-4 md:p-6 bg-muted/30">
          <div
            className={cn(
              "mb-3 md:mb-4 inline-block p-0 w-14 h-14 md:w-16 md:h-16 flex items-center justify-center",
        ...
        <CardContent className="flex-grow p-4 md:p-6 text-center">
          <p className="text-sm md:text-base text-foreground/70 leading-relaxed">{description}</p>
        </CardContent>
        <CardFooter className="p-4 md:p-6 pt-0 justify-center">
          <Button asChild variant="link" className="text-primary hover:text-primary/80 font-medium group">
            <Link href={link}>
